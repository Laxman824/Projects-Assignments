...
int main(int argc, char **argv)
{
    ...
    float q_density_sum = 0, dy_density_sum = 0;
    ...
    {   
        ...
        q_density_sum += q_density;
        dy_density_sum += dy_density;

        //writing the averaged queue & dynamic density per second (15 frames)
        if (c % 15 == 0)
        {
            //cout<<c<<endl;
            MyTextFile << (c / 15) << "," << q_density_sum / 15 << "," << dy_density_sum / 15 << endl;

            cout << (c / 15) << "," << q_density_sum / 15 << "," << dy_density_sum / 15 << endl;
